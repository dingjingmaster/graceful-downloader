...
char* conn_url (char *dst, size_t len, Conn *conn)
{
    ...
    int plen = snprintf(p, len, "%s%s%s:%i%s%s", prefix, conn->host, postfix, conn->port, conn->dir, conn->file);
...
int conn_setup (Conn *conn)
{
    ...
    if (PROTO_IS_FTP(conn->proto) && !conn->proxy) {
        ...
        for (int i = 0; i < conn->conf->add_header_count; i++) {
...
static int conn_info_ftp (Conn* conn)
{
